  ...
  if (error || !detail) {
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center p-6 text-center">
        ...
        <p className="text-textSecondary max-w-md mb-6">{error || '找不到匹配的沙箱轻小说项。'}</p>
        <button
          onClick={() => router.back()}
    ...
    >
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-6 py-12">
        ...
        <div className="flex flex-col md:flex-row gap-8 items-start mb-12">
          {/* Cover poster image */}
          <div className="animate-scale-in w-48 h-64 rounded-2xl overflow-hidden relative border border-borderSubtle shadow-2xl shadow-black/80 flex-shrink-0 mx-auto md:mx-0">
          ...
          <div className="flex-grow flex flex-col justify-between">
            <div>
              ...
              <div className="animate-fade-in flex flex-wrap gap-2 justify-center md:justify-start mb-6">
                ...
                {detail.genres?.map((genre, idx) => (
                  <span
                    key={idx}
                    className="px-3 py-1 rounded-full text-xs bg-surface border border-borderSubtle/80 text-textSecondary"
                  >
                    {genre}
                  </span>
                ))}
            ...
            <div className="animate-fade-in grid grid-cols-2 gap-4 border-t border-borderSubtle pt-6">
              <div className="flex items-center space-x-3 text-textSecondary">
                <div className="h-9 w-9 rounded-lg bg-surface border border-borderSubtle flex items-center justify-center">
                  <User className="h-4.5 w-4.5 text-violet-400" />
                </div>
                <div>
                  <div className="text-[10px] uppercase font-bold text-textTertiary tracking-wider">
                    作者
                  </div>
        ...
        <section className="bg-surface/20 border border-borderSubtle backdrop-blur-md rounded-3xl p-6 md:p-8">
          <div className="flex items-center space-x-3 border-b border-borderSubtle pb-4 mb-6">
            <BookOpen className="h-6 w-6 text-violet-500" />
            <h3 className="text-lg font-bold text-white">目录列表</h3>
            <span className="text-[10px] px-2 py-0.5 rounded bg-surfaceLight text-textSecondary font-bold border border-border/50">
              共 {chapters.length} 章
            </span>
          </div>

          {chapters.length === 0 ? (
            <div className="text-center py-12 text-textTertiary text-sm">暂无可用章节目录。</div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
              {chapters.map((chapter, idx) => (
                <a
                  ...
                  className="animate-chapter-btn flex items-center justify-between bg-background/60 border border-borderSubtle hover:border-borderSubtle/80 rounded-xl px-4 py-3.5 hover:bg-surface/30 active:scale-97 transition-all text-left text-sm group cursor-pointer"
                >
                  <div className="flex items-center gap-2.5 min-w-0">
                    ...
                    <span className="font-medium text-textSecondary group-hover:text-white transition-colors truncate">
                      {chapter.title}
                    </span>
                  </div>
                  <BookOpen className="h-4 w-4 text-textTertiary group-hover:text-violet-400 transition-colors flex-shrink-0" />
                </a>
              ))}
            </div>
          )}
