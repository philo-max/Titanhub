...
export default function MoviePlayPage({ params }: { params: Promise<Params> }) {
  ...
  const handleNextChapterClick = () => {
    if (nextChapter) {
      animatePlayPageExitNext(pageRef.current, () => {
        router.push(`/movie/${pluginId}/${mediaId}/play/${nextChapter.id}`);
      });
    }
  };
  ...
  const handleBack = (e: React.MouseEvent) => {
    ...
    animatePlayPageExitBack(pageRef.current, () => {
      router.push(`/movie/${pluginId}/${mediaId}`);
    });
  };
          ...
          <div className="flex flex-wrap gap-3">
            {chapters.map((ch) => {
              ...
              return (
                <Link
                  key={ch.id}
                  href={`/movie/${pluginId}/${mediaId}/play/${ch.id}`}
